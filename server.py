...
class LockdownRequest(BaseModel):
    ...
    provider: str = "auto"  # auto | openai | openrouter | llm | llm-realistic | mock
...
def run_lockdown(req: LockdownRequest):
    ...
    try:
        ...
        if req.provider in ("openai", "openrouter", "puter", "groq", "gemini", "genai") and backend_name == "mock":
...
def ai_capabilities():
    ...
    return {
        "status": "success",
        "priority_chain": ["openrouter", "puter", "openai", "pollinations", "deepai", "llm", "mock"],
        "providers": {
            "pollinations": {
                "available": True,
                "supports": ["image"],
...
async def stego_analyze_batch(
    ...
    try:
        ...
        for filename, data in items:
            ...
            try:
                packed_payload = _extract_payload_lsb_bytes(data)
                share_payload, decoder_report = _decode_payload_multiformat(packed_payload)
                if share_payload is None and packed_payload:
                    # Final fallback: try plain JSON text.
                    try:
                        share_payload = json.loads(packed_payload.decode("utf-8", errors="ignore").strip("\x00").strip())
                    except Exception:
                        share_payload = None
...
def stego_advanced_hide(req: AdvancedStegoHideRequest):
    ...
    try:
        ...
        if req.provider in ("genai", "openai", "openrouter", "puter", "groq") and backend_name == "mock":
            raise HTTPException(status_code=400, detail=f"Requested provider '{req.provider}' unavailable: {backend_reason}")
...
def russian_doll_fake_lsb_hide(req: RussianDollFakeLSBRequest):
    ...
    try:
        ...
        if req.provider in ("deepai", "openai", "openrouter", "puter", "groq", "gemini", "genai") and backend_name == "mock":
            raise HTTPException(status_code=400, detail=f"Requested provider '{req.provider}' unavailable: {backend_reason}")
...
async def face_verify(request: Request, images: List[UploadFile] = File(...)):
    try:
        ...
        if int(result.get("num_faces", 0)) != 1:
            raise HTTPException(status_code=400, detail="Exactly one face must be visible in every frame.")
...
async def embed_watermark(
    ...
    try:
        ...
        if logo_image is not None and logo_image.filename:
            ...
            logo_path = os.path.join(OUTPUT_DIR, logo_name)
        ...
        if doodle_image is not None and doodle_image.filename:
            ...
            doodle_path = os.path.join(OUTPUT_DIR, doodle_name)
...
def get_conversations(user_id: str):
    ...
    try:
        conversations = _conversation_manager.get_user_conversations(user_id)
